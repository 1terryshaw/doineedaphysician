...
export async function sendAddBusinessEmail(email: string, slug: string, token: string, businessName: string): Promise<AuthSendResult> {
  ...
  // (ruling R2, fan 2026-09-11) — a mail scanner prefetching this link now lands on a
  // confirmation page and changes nothing. See app/api/claim/verify/route.ts for why.
  const verifyLink = `${baseUrl}/claim/verify?token=${token}&slug=${slug}`;
  const name = businessName || "your business";
  try {
    const resend = new Resend(process.env.RESEND_API_KEY);
    const { data, error } = await resend.emails.send({
      from: AUTH_FROM, to: email,
      subject: `Confirm your email to finish your ${verticalConfig.name} listing`,
      ...
      html: `<h2>Verify your ${verticalConfig.name} listing</h2><p>Thanks for adding <strong>${name}</strong> to ${verticalConfig.name}. Click below to verify your email and finish your listing. The link expires in 24 hours.</p><p><a href="${verifyLink}" style="display:inline-block;padding:12px 24px;background:${verticalConfig.primaryColor};color:white;text-decoration:none;border-radius:6px;">Verify and continue</a></p><p>Or copy this link: ${verifyLink}</p><p style="color:#666;font-size:12px;">You received this because this email address was entered on ${String(baseUrl).replace(/^https?:\/\//, "").replace(/\/.*$/, "")}. If that wasn't you, ignore this email — nothing changes unless the link is used.</p><p style="color:#666;font-size:12px;">${verticalConfig.name} · owner mail from Do I Need A Network · <a href="https://doineedanetwork.com" style="color:#666;">doineedanetwork.com</a></p>`,
    });
    if (error) return { ok: false, error: error.message };
    return { ok: true, id: data?.id ?? "" };

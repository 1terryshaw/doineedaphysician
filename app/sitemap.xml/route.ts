...
async function renderSitemap() {
  ...
  const remainingChunks = Math.ceil(remainingListings / CHILD_SIZE);
  ...
  const childLastmod = (i: number): string | undefined => {
    const start = i === 0 ? 0 : firstChunkListingCapacity + (i - 1) * CHILD_SIZE;
    const end = i === 0 ? firstChunkListingCapacity : start + CHILD_SIZE;

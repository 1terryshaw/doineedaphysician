...
import { hasPublicStreet, isAddressPublic } from "@/lib/address-visibility";
...
export default async function ListingPage({ params }: Props) {
  ...
  // The geo node rides the PERMISSION, not `streetPublic`. A 12-decimal GeoCoordinates
  // point at street precision IS the address in machine-readable form, so suppressing the
  // string while emitting the point would be a gate cosmetically closed and materially
  // open. Gated on the permission alone because an empty `address` TEXT field must not
  // drop a coordinate the owner consented to publish.
  const geoPublic = isAddressPublic(lstAddr);
  ...
  const tierBadge = listing.subscription_tier
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            {/* Specialty badge */}
            {specialtyLabel && (
              <span className="inline-block text-sm px-3 py-1 rounded-full bg-green-50 text-green-700 font-medium mb-3">
                {specialtyLabel}
              </span>
            )}
            ...
            {(listing.city || streetPublic) && (
              <address className="not-italic text-gray-500 mb-3">
                ...
                {(listing.city) && (
                  <span className="block">
                    {listing.city}
                    {listing.province_state ? `, ${listing.province_state}` : ""}
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="border rounded-lg p-6 sticky top-4">
              ...
              (verticalConfig as { inquiryNoEmailPolicy?: "capture" | "hide" }).inquiryNoEmailPolicy === "capture" ? (
                <InquiryForm listingSlug={listing.slug} honest={!hasDeliverableEmail(listing)} />
              ) : (
                <ListingClaimCTA listingSlug={listing.slug} listingClaimed={listing.claimed} />
              )}
            </div>

            {/* Cross-referral sidebar */}
            {verticalConfig.crossReferrals.length > 0 && (
              <div className="border rounded-lg p-6">
                <h4 className="font-semibold text-gray-900 mb-3">You might also need:</h4>
                  ...
                  {verticalConfig.crossReferrals.map((ref) => (
                    <li key={ref.url}>
                      <a
                        href={
                          listing.region_slug
                            ? `${ref.url}${ref.pathPattern?.replace("{city}", listing.region_slug) ?? ""}`
                            : ref.url
                        }
                        target="_blank"
                        rel="noopener noreferrer"

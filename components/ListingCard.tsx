...
export default function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    ...
    >
      ...
      {specialtyLabel && (
        <span className="inline-block text-xs px-2 py-0.5 mt-2 rounded-full bg-green-50 text-green-700 font-medium">
          {specialtyLabel}
        </span>
      )}
        ...
        {listing.google_rating && (
          <span className="flex items-center gap-1">
            ...
            {listing.google_review_count && (
              <span className="text-gray-400">({listing.google_review_count})</span>
            )}
          </span>
        )}
